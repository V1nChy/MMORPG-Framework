...
namespace CodeX
{
    public class UpdateConfigStateListner : IGameStateListner
    {
        ...
        public override void OnStateEnter(GameState pCurState)
        {
            ...
            if (CheckUpdateManager.Instance.backupUrlState == BackUpUrlState.RUN)
            {
            ...
            MemoryLoadCallbackFunc delay_func = delegate (bool is_suc, byte[] buffer)
            {
                ...
                if (is_suc)
                {
                    ...
                    else
                    {
                        ...
                        {
                            CheckUpdateManager.Instance.backupUrlState = BackUpUrlState.NEW;
                            CheckUpdateManager.Instance.ToUpdateConfigState();
                            return;
                        }
                        if (CheckUpdateManager.Instance.backupUrlState == BackUpUrlState.RUN)
                        {
                            CheckUpdateManager.Instance.backupUrlState = BackUpUrlState.None;
        ...
        private void RetryLoadConfig()
        {
            ...
            if (CheckUpdateManager.Instance.ConfigTryLoadCount() < 2u)
            {
                CheckUpdateManager.Instance.TryLoadConfig();
                CheckUpdateManager.Instance.ToUpdateConfigState();
            }
            else
            {
                if (GameConfig.Instance.GetValue("BackUpUrl") != "" && CheckUpdateManager.Instance.backupUrlState == BackUpUrlState.None)
                {
                    CheckUpdateManager.Instance.backupUrlState = BackUpUrlState.RUN;
                    CheckUpdateManager.Instance.CleanTryLoadConfig();
                    CheckUpdateManager.Instance.ToUpdateConfigState();
                }
                else
                {
        ...
        public override void OnStateUpdate(GameState pCurState, float elapseTime)
        {
            if (this.m_finish_state)
            {
                ...
                CheckUpdateManager.Instance.ToCheckUpdateAppState();
            }
        }
